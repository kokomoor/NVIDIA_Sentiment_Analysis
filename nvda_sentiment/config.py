...
class Settings:
    ...
    sec_user_agent: str = "NVDA Sentiment Node kokomoor@mit.edu"
    sec_max_requests_per_second: float = 8.0  # SEC hard limit is 10/sec

    cache_dir: Path = field(default_factory=lambda: Path(".cache"))
    # Durable SQLite log of daily snapshots (§11.4 follow-up). Stable path — not
    # under cache_dir, so routine cache cleanup never wipes the history.
    history_db_path: Path = field(default_factory=lambda: Path("data/history.sqlite3"))
    # File-backed overlay for combiner knobs (Tier-2 tuning). Absent file ⇒ defaults apply.
    tuned_weights_path: Path = field(default_factory=lambda: Path("data/tuned_weights.json"))
    ...
    def __post_init__(self) -> None:
        ...
        self.tuned_weights_path = Path(self.tuned_weights_path)
